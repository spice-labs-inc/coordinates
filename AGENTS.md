...
Surveyor's `scripts/compare-test-ids.py` diffs those IDs against `tests/coverage/coordinates.tsv`
...
- Only canonical purls of real artifacts reach `spice` (in `purls.txt`), so most vectors are
